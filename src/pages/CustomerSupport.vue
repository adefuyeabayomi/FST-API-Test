        ...
        <div class="col-6 p-2">
          ...
          <div>
            <div class="border my-2 p-2">
              ...
              <pre>
{ "email": "customer@example.com", "message": "I need help." }</pre
              >
              ...
              <div class="form-group border p-2">
                ...
                <p></p>
                <Input
                  v-model="postMessage.message"
                  placeholder="Enter Message"
                />
                <p></p>
                <Button
                  @click="testPostCustomerMessage"
                  buttonVal="Test Post Customer Message"
                />
              </div>
            </div>
          </div>
          ...
          <div>
            <div class="border my-2 p-2">
              ...
              <p>
                <strong>Request Params:</strong> replied, startDate, endDate
              </p>
              <p><strong>Responses:</strong></p>
              <ul>
              ...
              <div class="form-group border p-2">
                <p>Filter Form</p>
                <Input
                  v-model="getMessages.replied"
                  placeholder="Replied (true/false)"
                />
                <p></p>
                <Input
                  v-model="getMessages.startDate"
                  placeholder="Start Date (YYYY-MM-DD)"
                />
                <p></p>
                <Input
                  v-model="getMessages.endDate"
                  placeholder="End Date (YYYY-MM-DD)"
                />
                <p></p>
                <Button
                  @click="testGetCustomerMessages"
                  buttonVal="Test Get Customer Messages"
                />
              </div>
            </div>
          </div>
          ...
          <div>
            <div class="border my-2 p-2">
              ...
              <div class="form-group border p-2">
                <p>Reply Form</p>
                <Input
                  v-model="replyMessage.messageId"
                ...
                <p></p>
                <Button
                  @click="testReplyToCustomerMessage"
                  buttonVal="Test Reply to Customer Message"
                />
              </div>
            </div>
          </div>
...
export default defineComponent({
  ...
  methods: {
    async testPostCustomerMessage() {
      try {
        const result = await customerSupportService.postCustomerMessage(
          this.postMessage.email,
    ...
    async testReplyToCustomerMessage() {
      try {
        const result = await customerSupportService.replyToCustomerMessage(
